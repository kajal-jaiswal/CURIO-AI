...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  const posts = await getPosts({ category: category.id, limit: 20 })
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      <header className="mb-8">
        <h1 className="text-4xl font-bold text-dark-50 mb-4">{category.name}</h1>
        {category.description && (
          <p className="text-lg text-dark-300">{category.description}</p>
        )}
      </header>

      {posts.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-dark-400 text-lg">No articles in this category yet.</p>
        </div>
      ) : (
